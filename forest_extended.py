...
class Forest(object):
    def __init__(self, n = 100, init_p = 0.59, bound_cond = "periodic", ignition_prob = 0.5):
        self.n = n
        self.init_p = init_p
    ...
    def set_ignition_prob(self,ignition_prob):
        self.ignition_prob=ignition_prob
    def get_n(self):
        return self.n
    def get_p(self):
                    ...
                    for dx in [-1,0,1]:
                        for dy in [-1,0,1]:
                            if self.state[ (x + dx) % self.n, (y + dy) % self.n ] == 2:
                                if pylab.random() < self.ignition_prob:
...
my_forests = [[Forest(100, 0.05+0.05*i, "periodic", 0.5) for j in range(5)] for i in range(20)]
...
def p_new(p_old,i):
    return p_old*p_old*i*(p_old*p_old*(-i**4+4*i**3-3*i**2+i)+2*p_old*(-i**2-1)+4)
k = 10
i = 0.5
p_old = 0.59
for c in range(k):
    current_p = p_new(p_old,i)
    p_old = current_p
    print current_p
