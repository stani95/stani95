...
def distr(x, y):
	return (0.35*norm.pdf(x, 1, 1.2)+0.65*norm.pdf(x, 6, 2.2))*(0.4*norm.pdf(y, 1, 1.4)+0.6*norm.pdf(y, 6, 2.7))
...
hist, xedges, yedges = np.histogram2d(samples_x, samples_y, bins=20, range=[[-5, 15], [-5, 15]], normed = True)
...
ax.plot_wireframe(x, y, distr(x,y), cmap=cm.coolwarm, linewidth=1., antialiased=False)
